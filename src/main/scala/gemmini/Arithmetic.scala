...
// 对不同类型提供统一的算术操作接口, 采用了scala的隐式类型类模型

// Bundles that represent the raw bits of custom datatypes
// expWidth: 指数位宽; sigWidth: 尾数位宽; isRecoded: 是否是 recoded 格式, 用于内部计算
// bias: 偏移量, 用于浮点运算
...
// 一个占位用的"假"有符号整数类型,用于调试或占位,不参与真实运算.
...
// 抽象类, 定义了可以对类型T执行哪些算术操作
// 使用 Scala 的 implicit 机制,为每种类型提供对应的 ArithmeticOps 实现.
abstract class Arithmetic[T <: Data] {
    implicit def cast(t: T): ArithmeticOps[T]
}

abstract class ArithmeticOps[T <: Data](self: T) {
    def *(t: T): T  // 每种操作都返回T类型, 保证链式调用的一致性
...
object Arithmetic {
    implicit object UIntArithmetic extends Arithmetic[UInt] {
        override implicit def cast(self: UInt) = new ArithmeticOps(self) {
            ...
            override def mac(m1: UInt, m2: UInt) = m1 * m2 + self  // 位宽规则遵循chisel的规则
            override def +(t: UInt) = self + t
            override def -(t: UInt) = self - t
            ...
            // 若 self 比 t 宽 → 直接砍掉高位(不饱和/不检查溢出).
            // 若 self 比 t 窄 → 高位补 0,扩展成与 t 同宽.
            override def withWidthOf(t: UInt) = self.asTypeOf(t)

            override def clippedToWidthOf(t: UInt) = {
                // 先Clamp到正上限,再砍,保证结果在 0^2^(n-1)-1 区间
    ...
    implicit object SIntArithmetic extends Arithmetic[SInt] {
        override implicit def cast(self: SInt) = new ArithmeticOps(self) {
            ...
            override def reciprocal[U <: Data](u: U, options: Int = 0): Option[(DecoupledIO[UInt], DecoupledIO[U])] =
                u match {
    ...
    implicit object FloatArithmetic extends Arithmetic[Float] {
        ...
        override implicit def cast(self: Float): ArithmeticOps[Float] = new ArithmeticOps(self) {
            override def *(t: Float): Float = {
                ...
                out.bits := (if (out.isRecoded) muladder.io.out
                             else fNFromRecFN(self.expWidth, self.sigWidth, muladder.io.out))
                out
            }
            ...
            override def +(t: Float): Float = {
                ...
                result.bits := (if (result.isRecoded) muladder.io.out
                                else fNFromRecFN(self.expWidth, self.sigWidth, muladder.io.out))
                result
            }
            ...
            override def withWidthOf(t: Float): Float = {
                ...
                result.bits := (if (result.isRecoded) resizer.io.out
                                else fNFromRecFN(t.expWidth, t.sigWidth, resizer.io.out))
                result
            }
            ...
            override def relu: Float = {
                val raw =
                    if (self.isRecoded) rawFloatFromRecFN(self.expWidth, self.sigWidth, self.bits)
                    else rawFloatFromFN(self.expWidth, self.sigWidth, self.bits)
    ...
    // 所有操作都返回 self.dontCare,用于占位或调试,不参与真实运算.
