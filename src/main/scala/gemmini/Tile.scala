...
/** A Tile is a purely combinational 2D array of passThrough PEs. a, b, s, and in_propag are broadcast across the entire
  * array and are passed through to the Tile's outputs
  * @param width
  *   The data width of each PE in bits
  * @param rows
  *   Number of PEs on each row
  * @param columns
  *   Number of PEs on each column
  */
class Tile[T <: Data](
    inputType: T,
    weightType: T,
    ...
    val tile = Seq.fill(rows, columns)(
        Module(new PE(inputType, weightType, outputType, accType, df, max_simultaneous_matmuls))
    )
    ...
    // tree_reduction = false 时直接取最底下 PE 的 out_b 当 io.out_b(c);
    // tree_reduction = true 时把整列所有 PE 的 out_b + 原始 in_b(c) 一起送进 accumulateTree
    // 做对数树求和, 用来支持 Reduce 型卷积而非纯脉动.
    for (c <- 0 until columns) {
        tileT(c).foldLeft(io.in_b(c)) { case (in_b, pe) =>
    ...
    for (c <- 0 until columns) {
        tileT(c).foldLeft(io.in_d(c)) { case (in_d, pe) =>
            pe.io.in_d := in_d
            pe.io.out_c
        }
    }
    ...
    for (c <- 0 until columns) {
        tileT(c).foldLeft(io.in_control(c)) { case (in_ctrl, pe) =>
    ...
    for (c <- 0 until columns) {
        tileT(c).foldLeft(io.in_valid(c)) { case (v, pe) =>
            pe.io.in_valid := v
            pe.io.out_valid
        }
    }
    ...
    for (c <- 0 until columns) {
        tileT(c).foldLeft(io.in_id(c)) { case (id, pe) =>
            pe.io.in_id := id
            pe.io.out_id
        }
    }
    ...
    for (c <- 0 until columns) {
        tileT(c).foldLeft(io.in_last(c)) { case (last, pe) =>
            pe.io.in_last := last
            pe.io.out_last
        }
    }
