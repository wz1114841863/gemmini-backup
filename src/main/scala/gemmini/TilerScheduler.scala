...
class TilerScheduler[T <: Data: Arithmetic, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V])(implicit
    val p: Parameters
) extends Module
    ...
    when(io.cmd_in.fire) {
        ...
        new_entry.dst.end := cmd.rs2(29, 0) +
            Mux(funct === PRELOAD_CMD, DIM.U, mvin_mvout_rows)
        ...
        new_entry.deps := (Cat(raws) |
            ...
            Cat(is_ex_config_and_must_wait_for_prior_st)).asBools.reverse
    ...
    Seq((ldq, io.issue.load), (stq, io.issue.store), (exq, io.issue.exec)).foreach { case (q, io) =>
        val issue_id = MuxCase(
            (ROB_ENTRIES - 1).U,
            entries.zipWithIndex.map { case (e, i) =>
                (e.valid && e.bits.ready() &&
                    !e.bits.issued && e.bits.q === q) -> i.U
            }
        )
        io.valid := entries
            .map(e =>
                e.valid && e.bits.ready() && !e.bits.issued
                    && e.bits.q === q
            )
            .reduce(_ || _)
    ...
    val util_ld_q_unissued = PopCount(
        entries.map(e =>
            e.valid &&
                !e.bits.issued &&
                e.bits.q === ldq
    ...
    val util_st_q_unissued = PopCount(
        entries.map(e =>
            e.valid &&
                !e.bits.issued &&
                e.bits.q === stq
    ...
    val util_ex_q_unissued = PopCount(
        entries.map(e =>
            e.valid &&
                !e.bits.issued &&
                e.bits.q === exq
