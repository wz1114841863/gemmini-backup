...
class TilerFSM[T <: Data: Arithmetic, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V])(implicit
    val p: Parameters
) extends Module
    ...
    val g_TILE_ROWS_PER_GROUP = MuxCase(
        ...
        OG_HEIGHT_MAP.zipWithIndex.map { case (h, i) =>
            (g_OG_DIM_SELECT(i) -> h.U)
        }
    )
    val g_TILE_COLS_PER_GROUP = MuxCase(
        (TOTAL_ACC_TILES / OG_HEIGHT_MAP(OG_HEIGHT_MAP.size - 1)).U,
        OG_HEIGHT_MAP.zipWithIndex.map { case (h, i) =>
            (g_OG_DIM_SELECT(i) -> (TOTAL_ACC_TILES / h).U)
        }
    )
    ...
    def update_tile_dims(dummy: Int = 0) = {
        gbl_item_rows := Mux(gbl_tile_row_n === g_TILE_ROW_END, g_LAST_M_ITEMS, DIM.U)
        gbl_item_cols := Mux(gbl_tile_col_n === g_TILE_COL_END, g_LAST_N_ITEMS, DIM.U)
        loop2_k_item_dims := Mux(loop2_k_tile_col_n === g_K_TILE_COL_END, g_LAST_K_ITEMS, DIM.U)
    }
    ...
    switch(state) {
        ...
        is(s_RESET_OUTPUT_GROUP) {
            ...
            loop1_tile_col_end := MIN(gbl_tile_col_n + g_TILE_COLS_PER_GROUP - 1.U, g_TILE_COL_END)
            loop1_tile_row_start := gbl_tile_row_n
            loop1_tile_row_end := MIN(gbl_tile_row_n + g_TILE_ROWS_PER_GROUP - 1.U, g_TILE_ROW_END)
        ...
        is(s_MAYBE_MOVE_NEXT_B_TILE_INTO_SP) {
            ...
            B_item_rows := loop2_k_item_dims
            B_item_cols := Mux(gbl_tile_col === g_TILE_COL_END - 1.U, g_LAST_N_ITEMS, DIM.U)
        ...
        is(s_PRELOAD_B_TILE_INTO_ARRAY_AND_SET_C_ADDR_IN_ACC) {
            ...
            val B_item_rows = Wire(UInt(16.W))
            val B_item_cols = Wire(UInt(16.W))
            B_sp_row_addr := Mux(gbl_tile_row === loop1_tile_row_start, gbl_B_cur_sp_row_addr, GARBAGE_ADDR)
            B_item_rows := loop2_k_item_dims
            B_item_cols := gbl_item_cols
            ...
            C_acc_row_addr := Mux(
                g_HAS_BIAS || (loop2_k_tile_col > 0.U),
        ...
        is(s_NEXT_A_TILE_SUBROW_IN_SUBCOL) {
            when(gbl_tile_row === loop1_tile_row_end) {
                ...
                loop4_D_mem_addr := loop4_D_mem_addr +
                    Mux(g_HAS_BIAS && !g_REPEATING_BIAS, g_D_BYTES_PER_TILE_ROW, 0.U)
        ...
        is(s_NEXT_OUTPUT_GROUP) {
            ...
            when(
                ...
                .otherwise {
                    ...
                    val l_tile_col_end = MIN(gbl_tile_col_n + g_TILE_COLS_PER_GROUP - 1.U, g_TILE_COL_END)
                    val l_tile_row_start = gbl_tile_row_n
                    val l_tile_row_end = MIN(gbl_tile_row_n + g_TILE_ROWS_PER_GROUP - 1.U, g_TILE_ROW_END)
