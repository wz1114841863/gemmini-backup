...
import freechips.rocketchip.tile._ // Rocket Chip组件

sealed abstract trait GemminiMemCapacity
case class CapacityInKilobytes(kilobytes: Int) extends GemminiMemCapacity // 以KB单位为
case class CapacityInMatrices(matrices: Int) extends GemminiMemCapacity // 以矩阵数量为单位
...
case class ScaleArguments[T <: Data, U <: Data](
    ...
    num_scale_units: Int,
    identity: String = "0",
    c_str: String = "ROUNDING_RIGHT_SHIFT(x, scale)"
)

/* 核心配置类, 定义整个systolic array的配置参数 */
case class GemminiArrayConfig[T <: Data: Arithmetic, U <: Data, V <: Data](
    ...
    weightType: T, // 权重数据类型
    accType: T, // 累加器数据类型
    spatialArrayInputType: T, // 脉动阵列输入数据类型
    spatialArrayWeightType: T, // 脉动阵列权重数据类型
    spatialArrayOutputType: T, // 脉动阵列输出数据类型

    dataflow: Dataflow.Value = Dataflow.BOTH, // 数据流类型

    // 硬件结构相关
    tileRows: Int = 1, // tileRows和tileColumns定义:
    tileColumns: Int = 1,
    meshRows: Int = 16, // meshRows和meshColumns定义:
    meshColumns: Int = 16,

    // 队列与流水线
    ld_queue_length: Int = 8, // load 指令队列长度
    st_queue_length: Int = 2, // store 指令队列长度
    ex_queue_length: Int = 8, // execute 指令队列长度
    reservation_station_entries_ld: Int = 8, // 保留站条目数
    reservation_station_entries_st: Int = 4,
    reservation_station_entries_ex: Int = 16,

    // 存储结构相关
    sp_banks: Int = 4, // TODO support one-bank designs,  scratchpad 的 bank 数
    ...
    // DMA 相关
    dma_maxbytes: Int = 64, // TODO get this from cacheblockbytes, 每次DMA传输的最大字节数
    dma_buswidth: Int = 128, // TODO get this from SystemBusKey, DMA数据总线宽度
    ...
    // ==========================================================================
    // sanity check mesh size, 网格尺寸合理性检查
    // ==========================================================================
    ...
    // ==========================================================================
    val res_max_per_type =
        max(reservation_station_entries_ld, max(reservation_station_entries_st, reservation_station_entries_ex))
    ...
    require(
        pow(2, I_TILE_BYTE_WIDTH_LOG2) == I_TILE_BYTE_WIDTH,
        s"I_TILE_BYTE_WIDTH is not power of 2: $I_TILE_BYTE_WIDTH"
    )
    require(
        pow(2, O_TILE_BYTE_WIDTH_LOG2) == O_TILE_BYTE_WIDTH,
    ...
    assert(USABLE_SP_TILES >= TOTAL_ACC_TILES, s"SP_TILES($USABLE_SP_TILES) + 2 < ACC_TILES($TOTAL_ACC_TILES)")
    ...
    require(
        isPow2(sp_bank_entries),
        "each SRAM bank must have a power-of-2 rows, to simplify address calculations"
    ) // TODO remove this requirement
    require(
        sp_bank_entries % (meshRows * tileRows) == 0,
        "the number of rows in a bank must be a multiple of the dimensions of the systolic array"
    )
    require(
        meshColumns * tileColumns == meshRows * tileRows,
        "the systolic array must be square"
    ) // TODO remove this requirement
    require(
        meshColumns * tileColumns >= 2,
        "the systolic array must have a dimension of at least 2"
    ) // TODO remove this requirement
    require(
        isPow2(meshColumns * tileColumns),
        "the systolic array's dimensions must be powers of 2"
    ) // TODO remove this requirement
    require(
        acc_bank_entries % (meshRows * tileRows) == 0,
        "the number of rows in an accumulator bank must be a multiple of the dimensions of the systolic array"
    )
    require(
        !mvin_scale_shared || (mvin_scale_shared && mvin_scale_args.isDefined && mvin_scale_acc_args.isEmpty && inputType.getWidth == accType.getWidth)
    ) // TODO is there a better way to check whether inputType and accType are the same?
    require(
        (mvin_scale_args.isEmpty || mvin_scale_acc_args.isEmpty) || (mvin_scale_t.getWidth == mvin_scale_acc_t.getWidth),
        "currently, the mvin scale types for both the srams and the accumulator must have the same width"
    ) // TODO remove this requirement

    def generateHeader(guard: String = "GEMMINI_PARAMS_H"): String = {
        // 根据当前配置生成 gemmini_params.h,供 C 模型/驱动/测试程序使用
        // 包括:数据类型定义, 阵列维度, 缩放与量化宏定义等
        def limitsOfDataType(dataType: Data): (String, String) = {
            ...
            dataType match {
                case dt: SInt =>
                    ("-" + BigInt(2).pow(dt.getWidth - 1).toString, BigInt(2).pow(dt.getWidth - 1).-(1).toString)
                case dt: DummySInt =>
                    ("-" + BigInt(2).pow(dt.getWidth - 1).toString, BigInt(2).pow(dt.getWidth - 1).-(1).toString)
                case dt: Float =>
                    (dt.expWidth, dt.sigWidth) match {
                        ...
                        case (e, s) =>
                            (
                                ((Range(-1, -(s), -1).map(-Math.pow(2, _)).foldLeft(-1.0)(_ + _)) * Math.pow(
                                    2,
                                    Math.pow(2, e - 1) - 1
                                )).toString,
                                ((Range(-1, -(s), -1).map(Math.pow(2, _)).foldLeft(1.0)(_ + _)) * Math.pow(
            ...
            dataType match {
                ...
                case dt: Float =>
                    (dt.expWidth, dt.sigWidth) match {
                        ...
                        case (11, 53) => "double"
                        case (e, s) =>
                            s"uint" + (Math.pow(2, Math.ceil(Math.log(e + s) / Math.log(2.0)))).toInt.toString + s"_t"
                    }
        ...
        if (
            inputType.isInstanceOf[Float] && !((
                inputType.asInstanceOf[Float].expWidth,
                inputType.asInstanceOf[Float].sigWidth
            ) == (8, 24) || (
                inputType.asInstanceOf[Float].expWidth,
                inputType.asInstanceOf[Float].sigWidth
            ) == (11, 53))
