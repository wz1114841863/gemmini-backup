...
/*
    转置预加载展开器, 用于识别并展开特定的预加载和计算指令序列
    作用: 优化预加载和计算指令的执行顺序, 提高数据处理效率
*/
class TransposePreloadUnroller[T <: Data, U <: Data, V <: Data](config: GemminiArrayConfig[T, U, V])(implicit
    p: Parameters
) extends Module {
    import config._
    import GemminiISA._
    ...
    when(io.out.fire) {
        when(is_config) {
            ...
            when(!set_only_strides) {
                b_transposed_and_ws := ((dataflow == Dataflow.WS).B || cmds(0).cmd.rs1(2) === Dataflow.WS.id.U) && cmds(
                    0
                ).cmd.rs1(9)
            }
...
object TransposePreloadUnroller {
    def apply[T <: Data, U <: Data, V <: Data](
        in: ReadyValidIO[GemminiCmd],
        config: GemminiArrayConfig[T, U, V],
        counter: CounterEventIO
